...
class ShuffleWaitPage(WaitPage):
    ...
    def after_all_players_arrive(self):
        ...
        if self.subsession.dice == 3:
            self.session.vars['second_max_round'] = self.subsession.round_number
            print('the final round is', self.session.vars['second_max_round'])
            self.session.vars['paying_round2'] = random.randint(1, self.session.vars['second_max_round'])
            print('the first payment round is', self.session.vars['paying_round2'])
...
class Shuffle(Page):
    ...
    def is_displayed(self):
        return self.subsession.is_playing() and self.subsession.round_number == 1
...
class SBRemainder(Page):
        ...
        def before_next_page(self):
            ...
            self.group.returned1_points = float(budget - self.group.kept_amount) * \
                                          float(self.group.send_back_amount1)/ 100
            self.group.returned2_points = float(budget - self.group.kept_amount) * \
                                          float(self.group.send_back_amount2)/ 100
            self.group.returned3_points = float(budget - self.group.kept_amount) * \
                                          float(self.group.send_back_amount3 / 100)
...
class DiceWaitPage(WaitPage):
    def after_all_players_arrive(self):
        ...
        if self.subsession.dice == 3:
            ...
            print('the second payment round is', self.session.vars['paying_round2'])
