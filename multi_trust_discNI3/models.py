...
class Constants(BaseConstants):
    name_in_url = 'multilateral_trust_dni3'
    players_per_group = 4
    num_rounds = 12
...
class Subsession(BaseSubsession):
    ...
    def creating_session(self):
        ...
        if self.round_number == 1:
            ...
            self.session.vars['matrix3'] = self.get_group_matrix()
            print('second group is', self.session.vars['matrix3'])
...
class Group(BaseGroup):
    ...
    send_back_amount1 = models.CurrencyField(min=0)
    send_back_amount2 = models.CurrencyField(min=0)
    send_back_amount3 = models.CurrencyField(min=0)
    ...
    def set_payoffs(self):
        ...
        for player in [distributor, sender1, sender2, sender3]:
            if self.subsession.round_number == self.session.vars['paying_round3']:
